...
def fillYCPT(bayesNet: BayesNet, gameState: GameState):
    ...
    填写给出 y 位置变量先验概率的 CPT。
    请参阅上文的 `fillXCPT` 定义，了解如何实现这一操作。
    您可以使用从布局中导入的 PROB_* 常量，而不是手工写下概率。
    """
...
def getMostLikelyFoodHousePosition(evidence: Dict, bayesNet: BayesNet, eliminationOrder: List[str]):
    ...
    寻找食物屋最可能的位置。
    首先，调用您刚实现的变量消除方法来获取 p(FoodHouse | everything else)。
    然后，检查得到的概率分布，找到食物屋最可能的位置。返回该位置。
    ...
    for assignment in Factor.getAllPossibleAssignmentDicts():
        if Factor.getProbability(assignment) > max_probability:
            max_probability = Factor.getProbability(assignment)
